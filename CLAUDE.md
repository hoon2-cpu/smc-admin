...
- [ ] **2) 외부업체 사진 표시 + 요청 클릭 팝업** — 팝업(상세 모달)은 쉬움. ⚠️ 사진 실표시는 **수리신청 사진을 Google Drive에 실제 업로드(DriveService)** 필요(현재는 파일명만 저장). 큰 작업.
- [ ] **3) 렌탈 자산 구분** — 대시보드에 구매/렌탈 별도 구분 + 렌트사별(롯데렌탈/**AJ네트웍스**) 집계. ※ 상수 `AI네트웍스→AJ네트웍스` 정정. 시트는 단일 `2_자산등록기록`+`렌탈사` 컬럼 유지 권장(물리적 시트 분리 여부 결정 필요).
- [ ] **4) 설정 모듈** — 사이드바 메뉴를 화면에서 추가/삭제(표시 토글). registry + 저장(로컬/시트).
- [ ] **5) 소모품 실데이터** — 구글시트에 IT소모품 리스트 시트 추가 → 대시보드 '소모품 재고 현황' 실연동.
...
- [ ] **9) 직원 소모품 신청 폼** — `/request`에 소모품신청 탭(마우스/키보드/카트리지 등). 총무팀이 구매/재고지급/외부업체(카트리지) 전달로 처리.
